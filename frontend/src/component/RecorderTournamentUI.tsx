...
export default function RecorderCompetitionUI({
  competitionName,
  scores,
  onApprove,
  onDisapprove,
          ...
          <table className="min-w-full table-auto">
            ...
            <thead>
              <tr className="bg-gray-100 text-sm text-gray-700">
                ...
                {/* Dynamic Round Groups */}
                {rounds.map((r) => {
                  const maxShots = maxShotsPerRound.get(r) || 0;
                  const colspan = maxShots + 2;
            ...
            <tbody className="text-sm">
              {pendingScores.map((s) => {
                const roundTotals = (s.details || []).reduce<
                    ...
                    {rounds.map((r) => {
                      ...
                      return (
                        <React.Fragment key={`row-${s.id}-r${r}`}>
                          ...
                          {Array.from({ length: maxShots }).map((_, i) => (
                            <td
                              key={`row-${s.id}-r${r}-s${i}`}
                              className="px-2 py-2 border text-center"
                            >
                              {rd && rd.shots[i] !== undefined
                                ? rd.shots[i]
                                : "-"}
                            </td>
                          ))}
                ...
                <tr>
                  <td
                    colSpan={
                      3 +
                      rounds.reduce(
                        (acc, r) => acc + (maxShotsPerRound.get(r) || 0) + 2,
                        0
                      ) +
                      3
                    }
                  ...
                  >
                    No pending submissions for this competition.
                  </td>
                </tr>
              )}
            </tbody>
